...
run.job <- function(cuffdiff.input, geneset.file, gtf.file, genome.file,  output.format,
   ...
   if (file_test("-f", cuffdiff.input)) {
      ...
      GP.CummeRbund.Geneset.Report(cuffdiff.job = getwd(), geneset.file, gtf.file, genome.file, output.format,
                                   feature.level, show.replicates, log.transform, cluster.count)
   } 
   else {
      ...
      if ("cuffData.db" %in% dir.contents) {
         ...
         tryCatch({
            GP.CummeRbund.Geneset.Report(cuffdiff.job = input.job, geneset.file, gtf.file, genome.file, output.format,
                                         feature.level, show.replicates, log.transform, cluster.count)
         },
         finally = {
